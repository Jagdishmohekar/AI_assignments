###implementing multinomial model
import string
import nltk
...
####calculating probability  given class DL and CV
...
for word in vocabalary:
    ...
    prob_word_given_cv = (num_word_given_cv + alpha) / (num_cv + alpha*vocab_len)
    prob_list_cv[word] = prob_word_given_cv

    num_word_given_dl = DL[word].sum() 
    prob_word_given_dl = (num_word_given_dl + alpha) / (num_dl + alpha*vocab_len)
...
def predict_class(D4):
    ...
    p2=prob_dl
    for word in D4:
        if word in prob_list_cv:
            p1 *= prob_list_cv[word]

        if word in prob_list_dl: 
